...
def main(args):
    ...
    old_feature = torch.tensor(np.array(np.load(cfg.old_embedding)))
    ...
    def weakened_L2loss(f1, f2, th):
        gap = f1 - f2
        loss_sum = torch.norm(gap, dim=1)
        loss_sum = loss_sum - th
        loss = torch.where(loss_sum < 0, torch.tensor(0.).cuda(), loss_sum)
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels, index) in enumerate(train_loader):
            global_step += 1
            old_local_embeddings = old_feature[index].cuda()
            local_embeddings = backbone(img)

            loss1: torch.Tensor = cfg.loss_weight[0] * module_partial_fc(local_embeddings, local_labels, opt)
            loss2 = cfg.loss_weight[1] * weakened_L2loss(F.normalize(old_local_embeddings), F.normalize(local_embeddings),cfg.threshold)
            loss = loss1 + loss2

            if global_step % 50 == 0 and rank == 0:
                print('CE loss: ', loss1.item())
                print('Feature L2 loss: ', loss2.item())
