...
class PartialFC(torch.nn.Module):
    ...
    def __init__(
        ...
        self._list_num_local = [num_classes // self.world_size + int(i < num_classes % self.world_size) for i in range(self.world_size)]
    ...
    def forward(
        ...
        if self.fp16:
            logits = logits.float()
            # total_logit = total_logit.float()

        logits = logits.clamp(-1, 1)
        # total_logit = total_logit.clamp(-1,1)

        logits = self.margin_softmax(logits, labels)

        #start
        _logits1 = logits[:,:min(self._list_num_local)].contiguous()
        if logits.shape[1] > min(self._list_num_local):
            _logits2 = logits[:, min(self._list_num_local):].contiguous()
        else:
            _logits2 = torch.zeros_like(logits[:, 0:1]).cuda()
        _list_logits1 = [torch.zeros_like(_logits1).float().cuda() for _ in range(self.world_size)]
        distributed.all_gather(_list_logits1, _logits1)
        _list_logits2 = [torch.zeros_like(_logits2).float().cuda() for _ in range(self.world_size)]
        distributed.all_gather(_list_logits2, _logits2)

        count = 0
        for i in self._list_num_local:
            if i > min(self._list_num_local):
                count += 1

        _list_logits = []
        for i in range(self.world_size):
            if i < count:
                _list_logits.append(torch.cat((_list_logits1[i], _list_logits2[i].reshape(-1, 1)), dim=1))
            else:
                _list_logits.append(_list_logits1[i])

        total_logit = torch.cat(_list_logits, dim=1)
        ...
        if return_loss:
            return total_logit, norm_embeddings, all_labels, loss
        else:
            return total_logit, norm_embeddings, all_labels
