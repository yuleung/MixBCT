...
from backbones import get_model_bct
...
def main(args):
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels, _) in enumerate(train_loader):
            ...
            BCT_loss = F.cross_entropy(bct_out, local_labels)
            loss = onehot_loss + BCT_loss

            if rank == 0 and global_step > 0 and global_step % 50 == 0:
                print(
                    'onehot_loss:{} BCT_loss:{} total_loss:{}'.format(onehot_loss.item(), BCT_loss.item(), loss.item()))

            #    for k,v in backbone.named_parameters():
            #        if k == 'module.BCT_FC.weight':
            #            print(v)
            if cfg.fp16:
