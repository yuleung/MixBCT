...
from backbones import get_model_softmax
...
def main(args):
    ...
    backbone = get_model_softmax(
        cfg.network, num_class=cfg.num_classes, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size
    ).cuda()
    if rank == 0:
        print(backbone)
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels, _) in enumerate(train_loader):
            global_step += 1
            x = backbone(img)
            loss = F.cross_entropy(x, local_labels)
            if cfg.fp16:
