...
from .iresnet_bct import iresnet18_bct, iresnet34_bct, iresnet50_bct, iresnet100_bct, iresnet200_bct

from .mobilefacenet import get_mbf

def get_model_bct(name, **kwargs):
    # resnet
    if name == "r18":
        return iresnet18_bct(False, **kwargs)
    elif name == "r34":
        return iresnet34_bct(False, **kwargs)
    elif name == "r50":
        return iresnet50_bct(False, **kwargs)
    elif name == "r100":
        return iresnet100_bct(False, **kwargs)
    elif name == "r200":
        return iresnet200_bct(False, **kwargs)
    elif name == "vit_s_dp005_mask_0":  # For WebFace42M
        num_features = kwargs.get("num_features", 512)
        syn_weight = kwargs.get('syn_weight')
        from .vit_bct import VisionTransformer_bct
        return VisionTransformer_bct(
            img_size=112, patch_size=9, num_classes=num_features, embed_dim=512, depth=12,
            num_heads=8, drop_path_rate=0.05, norm_layer="ln", mask_ratio=0.0, syn_weight = syn_weight)
    elif name == "vit_s":
        num_features = kwargs.get("num_features", 512)
        syn_weight = kwargs.get('syn_weight')
        from .vit_bct import VisionTransformer_bct
